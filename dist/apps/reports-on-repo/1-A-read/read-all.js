...
function readAll(commitOptions, clocParams) {
    ...
    const commitLogPath = (0, commit_1.writeCommitWithFileNumstat)(commitOptions);
    // execute the cloc commands
    const clocLogPath = (0, cloc_1.writeClocByfile)(clocParams, 'readAll-fileLinesOptions');
    const clocSummaryPath = (0, cloc_1.writeClocSummary)(clocParams);
...
function readAllParallel(commitOptions, clocParams) {
    const gitLogCommits = (0, commit_1.writeCommitWithFileNumstat$)(commitOptions);
    const cloc = (0, cloc_1.writeClocByFile$)(clocParams, 'cloc');
    const clocSummary = (0, cloc_1.writeClocSummary$)(clocParams);
...
function _streamsDistinctProcesses(commitOptions, clocParams, outGitFile, outClocSummaryFile) {
    ...
    const gitLogCommits = (0, commit_1.readCommitWithFileNumstat$)(commitOptions, outGitFile);
    // build the streams of cloc info
    const cloc = (0, cloc_1.clocByfile$)(clocParams, 'create cloc log', true);
    const clocSummary = (0, cloc_2.clocSummaryAsStreamOfStrings$)(clocParams, outClocSummaryFile, 'git');
...
function buildGitOutfile(config) {
    ...
    const _postfix = config.reverse ? commit_1.COMMITS_FILE_REVERSE_POSTFIX : commit_1.COMMITS_FILE_POSTFIX;
    const outFile = (0, cloc_1.buildOutfileName)(config.outFile, config.repoFolderPath, config.outFilePrefix, _postfix);
