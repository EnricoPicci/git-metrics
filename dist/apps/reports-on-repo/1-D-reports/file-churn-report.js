...
class FileChurnReport extends report_1.Report {
    constructor(_params) {
        super(_params, exports.FILE_CHURN_REPORT_NAME, `File churn report`);
...
function projectAndFileChurnReport(fileChurns, projectInfo, params, csvFilePath) {
    return projectInfo.pipe((0, rxjs_1.concatMap)((prjInfo) => fileChurnReport(fileChurns, params, prjInfo, csvFilePath)));
}
...
function fileChurnReport(fileChurns, params, projectInfo, csvFilePath) {
    return fileChurnReportCore(fileChurns, params, csvFilePath).pipe((0, rxjs_1.tap)((report) => {
        (0, add_project_info_1.addProjectInfo)(report, projectInfo, csvFilePath);
    }), (0, rxjs_1.map)((report) => addConsiderationsForFileChurnReport(report)));
}
exports.fileChurnReport = fileChurnReport;
...
function fileChurnReportCore(fileChurns, params, csvFilePath = '') {
    ...
    if (csvFilePath) {
        ...
        const _concurrentStreams = concurrentStreams;
        _concurrentStreams.push(allChurns);
        return (0, rxjs_1.forkJoin)(_concurrentStreams).pipe((0, rxjs_1.concatMap)(([report, allFileChurns]) => {
            report.csvFile.val = csvFilePath;
            const csvLines = mapFileChurnToCsv(allFileChurns, report);
            return (0, observable_fs_1.writeFileObs)(csvFilePath, csvLines).pipe((0, rxjs_1.map)((csvFile) => [report, csvFile]));
        }), (0, rxjs_1.tap)({
            next: ([, csvFile]) => {
                console.log(`====>>>> FILE CHURN REPORT GENERATED -- data saved in ${csvFile}`);
            },
        }), (0, rxjs_1.map)(([report]) => report));
    }
    return generateReport.pipe((0, rxjs_1.tap)({
        next: () => {
...
function _fileChurnReport(params) {
    return (0, rxjs_1.pipe)((0, rxjs_1.map)((filesInfo) => {
...
function mapFileChurnToCsv(fileChurns, report) {
    ...
    if (enrichedFileChurns.length === 0) {
        console.log('!!!!!!!! no data on file churns');
    }
    return (0, csv_tools_1.toCsv)(enrichedFileChurns);
}
