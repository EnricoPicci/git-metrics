...
const code_turnover_1 = require("../../git-cloc-functions/code-turnover");
function launchCodeTurnoverForRepos() {
    const start = Date.now();
    console.log('====>>>> Launching code-turnover For Repos');
    const { folderPath, outdir, fromDate, toDate, excludeRepoPaths, languages, removeBlanks, removeComments, removeSame, fileMassiveRefactorThreshold, commitMassiveRefactorThreshold, commitMassiveRemoveThreshold, jiraIdRegexPattern, creationDateCsvFilePath, notMatchDirectories } = readParams();
    const options = {
        outdir,
        fromDate,
        toDate,
...
exports.launchCodeTurnoverForRepos = launchCodeTurnoverForRepos;
function readParams() {
...
(wildcards are allowed).`)
        .requiredOption('--folderPath <string>', `folder containing the repos to analyze (e.g. ./my-repos)`)
...
- default is the empty list which means all languages`)
        ...
        .option('--fileMassiveRefactorThreshold <number>', `if this opion is specified, the flag to indicate whether a file diff is likely derived from a massive refactoring will be calculated (the logic being that a diff on a file with a code turnover higher than the threshold is likely to be a massive refactoring)`)
        .option('--commitMassiveRefactorThreshold <number>', `if this opion is specified, the flag to indicate whether a file diff is likely derived from a massive refactoring will be calculated (the logic being that a diff belonging to a commit whose code-turnover higher than the threshold is likely to be a massive refactoring)`)
        .option('--commitMassiveRemoveThreshold <number>', `if this opion is specified, the flag to indicate whether a file diff is likely derived from a massive removal will be calculated (the logic being that a diff belonging to a commit whose code-turnover is mainly a massive removal can be filtered out from the code turnover analysis)`)
        .option('--jiraIdRegexPattern <string>', `regex to extract the Jira ID from the commit subject (e.g. "[A-Za-z]+-\\d+|\\[[A-Za-z]+ \\d+\\]|[A-Za-z]+ +- \\d" will extract Jira IDs of the form "XXXX-1234" or "[XXXX 1234]" or "XXXX - 1234")`)
...
- created_at: the date of creation of the repo`)
        .option('--notMatchDirectories <string...>', `a space separated list of regular expression that are evaluated against the directories of the files in the commit (e.g. --notMatchDirectories "node_modules"). If the regular expression is matched, the file excluded.`);
