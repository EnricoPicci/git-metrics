...
describe(`readTagsCommand`, () => {
    it(`builds the git log command to read the tags`, () => {
        ...
        const expected = `git -C ${config.repoFolderPath} log --no-walk --tags --pretty='${tag_1.SEP}%h${tag_1.SEP}%d${tag_1.SEP}%s' --decorate=full > ${expectedOutfile}`;
        const [cmd, out] = (0, tag_1.readTagsCommand)(config);
...
describe(`readTags`, () => {
    it(`read the tags from a git repo using git log command and saves them in a file`, (done) => {
        ...
        const returnedOutFilePath = (0, tag_1.readTags)(config);
