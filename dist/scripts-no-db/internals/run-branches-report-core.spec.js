...
describe(`runBranchesReport`, () => {
    it(`runs branches report on this project`, (done) => {
        ...
        const runSingleStream = (0, run_branches_report_core_1.runBranchesReport)(repoFolderPath, after, outDir, outFile, clocDefsPath, false);
        runSingleStream
            .pipe((0, rxjs_1.tap)((reports) => {
            (0, chai_1.expect)(reports.length).equal(1);
            (0, chai_1.expect)(reports[0]).not.undefined;
        }))
            .subscribe({
            error: (err) => done(err),
            complete: () => done(),
