...
import { copyRenamesDict$, diff$, splitDiffs } from "./diff-file";
...
describe('diff$', () => {
    ...
    it('should build a stream that notifies a mix of copy/rename and non copy/renama diffs', (done) => {
        ...
        //     '15\t0\tsrc/config-copy-yy.ts',
        //     '14\t0\tsrc/git-functions/diff-file.spec.ts',
        //     '9\t4\tsrc/git-functions/diff-file.ts',
        //     '1\t0\tsrc/xyz.md'
        ...
        diff$(commit, parentCommit, repoPath).pipe(
            tap((diffs) => {
                ...
                expect(lastDiff.linesAdded).equal(1);
...
describe('copyRenamesDict$', () => {
    ...
    of the only one diff which is a rename/copy`, (done) => {
        ...
        copyRenamesDict$(commit, parentCommit, repoPath).pipe(
            tap((diffsDict) => {
                expect(Object.keys(diffsDict).length).equal(2);
                expect(diffsDict[expectedFilePath].filePath).equal(expectedFilePath);
                expect(diffsDict[expectedFilePath].preImagePath).equal(expectedPreImagePath);
            })
        ).subscribe({
            error: (err) => done(err),
            complete: () => done()
