 ...
 */
export function cloneRepo$(url: string, repoPath: string, options: ExecuteCommandObsOptions = {}) {
    ...
    return executeCommandObs$(`Clone ${repoName}`, command, options).pipe(
        tap(() => `${repoName} cloned`),
        ignoreElements(),
 ...
 */
export function pullAllRepos$(folderPath: string, concurrency = 1, excludeRepoPaths: string[] = [], options: ExecuteCommandObsOptions = {}) {
 ...
 * @throws An error if the repoPath parameter is not provided.
 */
export function fetchRepo$(repoPath: string, options: ExecuteCommandObsOptions = {}) {
    if (!repoPath) throw new Error(`Path is mandatory`);

    const repoName = path.basename(repoPath);
    const command = `cd ${repoPath} && git fetch --all`;

    return executeCommandObs$(`Fetch ${repoName}`, command, options).pipe(
 ...
 */
export function fetchAllRepos$(folderPath: string, concurrency = 1, excludeRepoPaths: string[] = [], options: ExecuteCommandObsOptions = {}) {
    const repoPaths = gitRepoPaths(folderPath, excludeRepoPaths);
...
export function repoPathAndFromDates$(repoPaths: string[], fromDate: Date, creationDateCsvFilePath: string | null, options: ExecuteCommandObsOptions = {}) {
    ...
    return _repoCreationDateDict$.pipe(
        concatMap((dict) => {
            return from(repoPaths).pipe(
                concatMap((repoPath) => getRemoteOriginUrl$(repoPath, options).pipe(
                    map((remoteOriginUrl) => {
