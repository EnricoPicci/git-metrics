import { pipe, Observable, forkJoin, map, share, tap, concatMap } from 'rxjs';
...
export class FileChurnReport extends Report {
    ...
    constructor(_params: FileChurnReportParams) {
        super(_params, FILE_CHURN_REPORT_NAME, `File churn report`);
    }
    addConsiderations() {
...
export function fileChurnReportCore(
    ...
    if (csvFilePath) {
        ...
        const _concurrentStreams = concurrentStreams as [Observable<FileChurnReport>, Observable<FileChurn[]>];
...
export function addConsiderationsForFileChurnReport(r: FileChurnReport) {
    ...
    addConsideration(
        r,
        `The files which have been changed in the period currently contain ${r.clocTot.val
    ...
    addConsideration(
        r,
        `${((r.topChurnContributors.val.length / r.numFiles.val) * 100).toFixed(2)}% of files contribute to ${(r.params.val as any)['percentThreshold']
        }% of churn`,
    );
    addConsideration(
        r,
